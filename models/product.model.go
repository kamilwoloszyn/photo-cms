...
func (p *Product) GetCustomerDetails(c *Customer) error {
	...
	if customerId := checkers.UuidGeneric(p.CustomerId); customerId.IsEmpty() {
		return ErrIdEmpty
	}
...
func (p *Product) GetOrderDetails(o *Order) error {
	...
	if orderId := checkers.UuidGeneric(p.OrderId); orderId.IsEmpty() {
		return ErrIdEmpty
	}
	o.ID = p.OrderId
...
func (p *Product) GetCategoryDetails(c *Category) error {
	...
	if categoryId := checkers.UuidGeneric(p.CategoryId); categoryId.IsEmpty() {
		return ErrIdEmpty
	}
...
func (p *Product) GetImageDetails(i *Image) error {
	...
	if imageId := checkers.UuidGeneric(p.ImageId); imageId.IsEmpty() {
		return ErrIdEmpty
	}
	i.SetID(p.ImageId)
...
func (p *Product) GetProductOptions() error {
	...
	tx := handler.Model(p).Select("products.id,product_options.id,product_options.created_at,product_options.updated_at,product_options.deleted_at,product_options.option_value_id,product_options.product_id").Joins("left join product_options on product_options.id=products.id").Where("products.id=?", p.GetID()).Find(&p.ProductOption)
	if tx.Error != nil {
